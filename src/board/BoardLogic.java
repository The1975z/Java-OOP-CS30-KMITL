...
public class BoardLogic {
    ...
    private boolean gameOverHandled = false; // Add a flag to track if game over was already handled
    ...
    private int calculateTimeSpent() {
        ...
        long timeDiffMillis = currentTime - moveStartTime;
        int timeSpentSeconds = (int) (timeDiffMillis / 1000); 
    ...
    public void movePiece(int toRow, int toCol) {
        ...
        if (isValidMove(selectedRow, selectedCol, toRow, toCol)) {
            ...
            if (board.getGameControllerObj() != null) {
                ...
                GameFrame frame = (GameFrame) SwingUtilities.getWindowAncestor(board);
                if (frame != null) {
                    int timeSpentSeconds = calculateTimeSpent(); 
                    frame.recordMove(selectedRow, selectedCol, toRow, toCol, previousTurn, timeSpentSeconds);
                    
                    // รีเซ็ต moveStartTime เพื่อเริ่มจับเวลาใหม่สำหรับผู้เล่นถัดไป
                    moveStartTime = System.currentTimeMillis();
                    
                    // ตรวจสอบว่า timer ยังทำงานอยู่หลังจากลงหมากสีดำ
                    if (!previousTurn) {
                        System.out.println("After BLACK move - ensuring timer is running");
                        frame.getTimerManager().ensureTimerRunning();
                    }
                }
            }
            ...
            // Additional check for game over after every move
            ...
            moveStartTime = -1; // รีเซ็ตเวลาหากยกเลิกการเลือก
        }
        ...
        for (int[] dir : directions) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (hasSameColorAtEnd && !opponentPieces.isEmpty()) {
                    ...
                    if (opponentColor == RokPiece.Color.WHITE) {
                        board.setBlackCaptured(board.getBlackCaptured() + opponentPieces.size()); // ดำจับขาวได้
                        board.setWhiteRemaining(board.getWhiteRemaining() - opponentPieces.size()); // ขาวลดลง
                    } else {
                        board.setWhiteCaptured(board.getWhiteCaptured() + opponentPieces.size()); // ขาวจับดำได้
                        board.setBlackRemaining(board.getBlackRemaining() - opponentPieces.size()); // ดำลดลง
                    }
                    captured = true;
                }
            }
    ...
    public void checkAtari() {
        ...
        // Check if the game is over after captures
        checkGameOverCondition();
    }

    private void checkGameOverCondition() {
        if (gameOverHandled) {
            return; // Skip if game over has already been handled
        }
        
        if (isGameOver()) {
            String winner = getWinner();
            if (!winner.equals("ONGOING")) {
                // Get access to the GameFrame to stop the timer
                GameFrame frame = (GameFrame) SwingUtilities.getWindowAncestor(board);
                if (frame != null && frame.getTimerManager() != null) {
                    frame.getTimerManager().stopGameTimer();
                    System.out.println("Game over detected. Timer stopped.");
                }
                
                // Important: Notify the game controller that the game is over
                if (board.getGameControllerObj() != null) {
                    gameOverHandled = true; // Mark as handled to prevent duplicate dialogs
    ...
    public boolean isGameOver() {
        ...
        System.out.println("Remaining pieces: White=" + whiteCount + ", Black=" + blackCount);
    ...
    public void resetGame() {
        ...
        // Reset the game over flag when resetting the game
