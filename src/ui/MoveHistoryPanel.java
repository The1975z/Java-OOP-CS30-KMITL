...
public class MoveHistoryPanel extends JPanel {
    ...
    private JPanel createColumnHeaders() {
        JPanel headerRow = new JPanel(new GridLayout(1, 4, 10, 0)); // Changed from 5 to 4 columns
        headerRow.setBackground(HEADER_COLOR);
        headerRow.setBorder(new EmptyBorder(10, 15, 10, 15));

        // เปลี่ยนหัวตารางให้มีแค่คอลัมน์ Time Used แทน W/B Time Used
        String[] headers = {"#", "White", "Black", "Time Used"};
    ...
    private JPanel createMoveRow(MoveRecord move) {
        JPanel moveRow = new JPanel(new GridLayout(1, 4, 10, 0)); // Changed from 5 to 4 columns
        ...
        // เพิ่มการตรวจสอบว่าเป็นการเดินของฝ่ายใด
        boolean isWhiteMove = !move.whiteMoveFrom.equals("-") && !move.whiteMoveTo.equals("-");
        boolean isBlackMove = !move.blackMoveFrom.equals("-") && !move.blackMoveTo.equals("-");
        ...
        // 4. คอลัมน์ที่สี่: รวมเวลาที่ใช้ทั้ง 2 ฝ่าย - แสดงเฉพาะตัวเลขเวลาไม่มีคำนำหน้า
        String timeText;
        if (isWhiteMove) {
            timeText = formatTimeDisplay(move.whiteTimeTaken);
        } else if (isBlackMove) {
            timeText = formatTimeDisplay(move.blackTimeTaken);
        } else {
            timeText = "-";
        }
        
        JLabel timeLabel = new JLabel(timeText);
        timeLabel.setForeground(new Color(255, 220, 150));
        timeLabel.setFont(new Font("Arial", Font.BOLD, 14));
        timeLabel.setHorizontalAlignment(SwingConstants.CENTER);
        moveRow.add(timeLabel);
