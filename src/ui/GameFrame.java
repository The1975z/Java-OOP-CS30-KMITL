...
public class GameFrame extends JFrame {
    ...
    public void recordMove(int fromRow, int fromCol, int toRow, int toCol, boolean isWhiteMove, int timeSpent) {
        if (historyPanel != null) {
            ...
            // คำนวณเวลาที่ใช้ในตานี้ (เฉพาะตานี้)
            int whiteMoveTime = whiteUsedTime - previousWhiteUsedTime;
            int blackMoveTime = blackUsedTime - previousBlackUsedTime;
            
            System.out.println("  Previous used times - White: " + previousWhiteUsedTime + "s, Black: " + previousBlackUsedTime + "s");
            System.out.println("  Move time this turn - White: " + whiteMoveTime + "s, Black: " + blackMoveTime + "s");
            
            // For the first move, reset previous times
            if (historyPanel.isFirstMove()) {
                ...
                whiteMoveTime = whiteUsedTime;
                blackMoveTime = blackUsedTime;
                System.out.println("  First move - initializing times");
            }
            
            // อัปเดตค่าเวลาล่าสุดสำหรับการคำนวณในตาถัดไป
            if (isWhiteMove) {
                previousWhiteUsedTime = whiteUsedTime;
            } else {
                previousBlackUsedTime = blackUsedTime;
            }
            ...
            if (isWhiteMove) {
                ...
                historyPanel.addGameMove(
                    ...
                    opponentToCol,
                    whiteMoveTime,       // เวลาที่ขาวใช้ในตานี้
                    blackMoveTime        // เวลาที่ดำใช้ในตาก่อน
                );
                ...
                historyPanel.addGameMove(
                    ...
                    toCol,
                    blackMoveTime,       // เวลาที่ดำใช้ในตานี้ (ส่งในตำแหน่งแรก)
                    whiteMoveTime        // เวลาที่ขาวใช้ในตาก่อน (ส่งในตำแหน่งที่สอง)
                );
